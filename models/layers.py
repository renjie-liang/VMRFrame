...
class FeatureEncoder(nn.Module):
    def __init__(self, dim, num_heads, max_pos_len, kernel_size=7, num_layers=4, droprate=0.0):
        ...
        self.attention_block = MultiHeadAttentionBlock(dim=dim, num_heads=num_heads, droprate=droprate)
...
class SeqPANPredictor(nn.Module):
    def __init__(self, configs):
        ...
        self.feature_encoder = FeatureEncoder(dim=configs.model.dim, num_heads=configs.model.num_heads, kernel_size=1, num_layers=2,
                                              max_pos_len=configs.max_pos_len, droprate=configs.model.droprate)

        self.layer_norm = nn.LayerNorm(configs.model.dim, eps=1e-6)
    ...
    def forward(self, x, mask):
        ...
        start_feat = self.layer_norm(start_feat)
        end_feat = self.layer_norm(end_feat)
